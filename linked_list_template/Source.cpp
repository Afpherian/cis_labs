...
bool LinkedList<T>::remove(T searchValue) {
	...
	ListNode<T> *previousNode=nullptr; //to point to previous node
...
int main() {
	...
	LinkedList<string> list; //define a LinkedList to hold string values
	...
	do {
		...
		if (choice == 5){
			...
			cin.get();
			getline(cin, new_value);
			isfound = list.contains(new_value);
			if (isfound)
